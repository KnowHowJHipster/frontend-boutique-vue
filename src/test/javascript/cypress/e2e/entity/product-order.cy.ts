...
describe('ProductOrder e2e test', () => {
  ...
  // const productOrderSample = {"quantity":6132,"totalPrice":30941.33};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/products',
      body: {"name":"embargo surprisingly dawdle","description":"by","price":19260.91,"productSize":"XXL","image":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","imageContentType":"unknown"},
    }).then(({ body }) => {
      product = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2024-06-03T23:41:59.850Z","status":"PENDING","totalPrice":7133.22,"paymentMethod":"CREDIT_CARD","paymentReference":"deprive whose what"},
    }).then(({ body }) => {
      shoppingCart = body;
    });
  });
  ...
  describe('new ProductOrder page', () => {
    ...
    it.skip('should create an instance of ProductOrder', () => {
      cy.get(`[data-cy="quantity"]`).type('3750');
      cy.get(`[data-cy="quantity"]`).should('have.value', '3750');

      cy.get(`[data-cy="totalPrice"]`).type('489.69');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '489.69');
