...
describe('ShoppingCart e2e test', () => {
  ...
  // const shoppingCartSample = {"placedDate":"2024-06-04T04:21:05.024Z","status":"PAID","totalPrice":25173.44,"paymentMethod":"IDEAL"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {"gender":"MALE","phone":"889-613-7463","addressLine1":"delve cricketer","addressLine2":"except mmm neatly","city":"East Susiebury","country":"North Macedonia"},
    }).then(({ body }) => {
  ...
  describe('new ShoppingCart page', () => {
    ...
    it.skip('should create an instance of ShoppingCart', () => {
      cy.get(`[data-cy="placedDate"]`).type('2024-06-04T03:20');
      cy.get(`[data-cy="placedDate"]`).blur();
      cy.get(`[data-cy="placedDate"]`).should('have.value', '2024-06-04T03:20');

      cy.get(`[data-cy="status"]`).select('REFUNDED');

      cy.get(`[data-cy="totalPrice"]`).type('7979.12');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '7979.12');

      cy.get(`[data-cy="paymentMethod"]`).select('IDEAL');

      cy.get(`[data-cy="paymentReference"]`).type('dismount extremely reduplicate');
      cy.get(`[data-cy="paymentReference"]`).should('have.value', 'dismount extremely reduplicate');
